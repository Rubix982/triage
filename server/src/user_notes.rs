    ...
    with_connection("get_views", |conn| {
        ...
        let rows = stmt.query_map([&user_id], |row| {
            let view = SavedView {
                ...
                view_type: serde_json::from_str(&row.get::<_, String>(3)?).unwrap_or(ViewType::Search),
                configuration: serde_json::from_str(&row.get::<_, String>(4)?).unwrap_or_default(),
...
pub async fn toggle_view_favorite(view_id: &str) -> Result<bool, String> {
    ...
    with_connection("toggle_favorite", |conn| {
        ...
        let mut stmt = conn.prepare("SELECT is_favorite FROM saved_views WHERE id = ?1")?;
        let current_state = stmt.query_row([view_id], |row| {
            Ok(row.get::<_, i32>(0)? != 0)
        }).map_err(|e| format!("Failed to get current favorite state: {}", e))?;
...
fn build_note_from_row(row: &rusqlite::Row) -> Result<UserNote, rusqlite::Error> {
    Ok(UserNote {
        id: row.get(0)?,
        title: row.get(1)?,
        content: row.get(2)?,
        note_type: serde_json::from_str(&row.get::<_, String>(3)?).unwrap_or(NoteType::PersonalNote),
        tags: serde_json::from_str(&row.get::<_, String>(4).unwrap_or_default()).unwrap_or_default(),
        linked_items: serde_json::from_str(&row.get::<_, String>(5).unwrap_or_default()).unwrap_or_default(),
        created_at: row.get(6)?,
        updated_at: row.get(7)?,
        user_id: row.get(8)?,
        is_private: row.get::<_, i32>(9)? != 0,
        metadata: serde_json::from_str(&row.get::<_, String>(10).unwrap_or_default()).unwrap_or(NoteMetadata {
            color: None,
            priority: 3,
            completion_status: CompletionStatus::NotStarted,
            estimated_time: None,
